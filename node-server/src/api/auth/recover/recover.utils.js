class RecoverUtils {
  constructor (dependencies) {
    ...
    this.generateCode = dependencies.generateCode
    this.generateHTML = dependencies.generateHTML
    // errors
    this.EmailSendError = dependencies.EmailSendError
    this.ValidationError = dependencies.ValidationError
    this.ExpirationError = dependencies.ExpirationError
  }

  expirationTime () {
    const time = this.auth.codeExpiration // minutes to expiration
    return new Date(Date.now() + (time * 60 * 1000))
  }

  async createAndSaveCode (userId) {
    const code = await this.generateCode()
    const recoveryCodeData = { userId, code, expiresAt: this.expirationTime() }
    await this.RecoveryCodeModel.create(recoveryCodeData)
    return code
  }

  async sendEmailCode (data = { code: null, email: null }) {
    const html = await this.generateHTML('emails/recoveryCode', { code: data.code })
    const options = {
      to: data.email,
    ...
    const emailResponse = await this.sendEmail(options)
    if (emailResponse.error) {
      throw new this.EmailSendError(`Error sending email: ${emailResponse.error?.message}`)
    }
  ...
  async sendEmailPasswordSuccess (email) {
    const html = await this.generateHTML('emails/recoveryPassword')
    const options = {
      to: email,
  ...
  async verifyAndGetUser (email) {
    const user = await this.UserModel.findOneData({ email })
    if (!user) { throw new this.ValidationError(`The email ${email} does not correspond to any registered user`) }
    return user
  }

  async verifyRecoveryCode ({ code, userId }) {
    const recoveryCode = await this.RecoveryCodeModel.findOneData({ code, userId })

    if (!recoveryCode) { throw new this.ValidationError(`Code ${code} not found, please generate a new one`) }

    if (recoveryCode.expiresAt < new Date()) { throw new this.ExpirationError(`Code ${code} has expired, please generate a new one`) }
  }
}

module.exports = RecoverUtils
